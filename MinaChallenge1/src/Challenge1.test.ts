...
describe('MinaChallange1', () => {
    ...
    it('generates and deploys the `MinaChallange1` smart contract', async () => {
        await localDeploy();
        const admin = zkApp.adminPubKey.get();
    ...
    it('correctly adds the new address as action on the `MinaChallange1` smart contract', async () => {
        ...
        const txn = await Mina.transaction(deployerAccount, () => {
            zkApp.storeAddress(senderAccount, adminKey);
        });
        await txn.prove();
        ...
        const txn2 = await Mina.transaction(deployerAccount, () => {
            zkApp.storeAddress(bobAccount, adminKey);
        });
        await txn2.prove();
    ...
    it('correctly adds the new message as action on the `MinaChallange1` smart contract', async () => {
        ...
        const txn2 = await Mina.transaction(deployerAccount, () => {
            ...
            // 0b10000000
            zkApp.storeMessage(Field(0b11000000), senderKey);
        });
        await txn2.prove();
    ...
    it('correctly checks if message format is right', async () => {
        ...
        await Mina.transaction(deployerAccount, () => {
            ...
            // 0b10000000
            zkApp.isMessageValid(Field(0b1111111));
        }).catch((e) => (errorMsg = e));
        ...
        await Mina.transaction(deployerAccount, () => {
            // Create a field with last 6 bits are not following the message format rules
            // 0b10000000
            zkApp.isMessageValid(Field(0b11000000));
        }).catch((e) => (errorMsg = e));
        ...
        await Mina.transaction(deployerAccount, () => {
            // Create a field with last 6 bits are not following the message format rules
            // 0b10000000
            zkApp.isMessageValid(Field(0b11111110000111));
        }).catch((e) => (errorMsg = e));
        expect(errorMsg).toEqual(Error('Bool.assertEquals(): true != false'));
        ...
        await Mina.transaction(deployerAccount, () => {
            // Create a field with last 6 bits are not following the message format rules
            // 0b10000000
            zkApp.isMessageValid(Field(0b1111111000110));
        }).catch((e) => (errorMsg = e));
        expect(errorMsg).toEqual(Error('Bool.assertEquals(): true != false'));
        ...
        await Mina.transaction(deployerAccount, () => {
            // Create a field with last 6 bits are not following the message format rules
            // 0b10000000
            zkApp.isMessageValid(Field(0b11111110011000));
        }).catch((e) => (errorMsg = e));
        expect(errorMsg).toEqual('');
        errorMsg = '';
        await Mina.transaction(deployerAccount, () => {
            // Create a field with last 6 bits are not following the message format rules
            // 0b10000000
            zkApp.isMessageValid(Field(0b11111110010000));
        }).catch((e) => (errorMsg = e));
        expect(errorMsg).toEqual(Error('Bool.assertEquals(): true != false'));
    });
});
